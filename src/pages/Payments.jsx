    ...
    return (
        <>
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Candidate Payments</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Manage payments from candidates</p>
                </div>
                <button
            ...
            <Table headers={['Date', 'Candidate', 'Type', 'Amount', 'Account', 'QR']}>
                {candidatePayments.map(p => (
                    <TableRow key={p.id} className="dark:hover:bg-gray-700/50">
                        ...
                        <TableCell>
                            <div className="font-bold text-gray-800 dark:text-gray-200">{p.Candidate?.name}</div>
                            <div className="text-[10px] text-brand-600 dark:text-brand-400 font-bold">{p.Candidate?.candidate_code}</div>
                        </TableCell>
                        <TableCell><span className="text-xs bg-brand-50 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300 px-2 py-1 rounded font-medium">{p.CandidatePaymentType?.name}</span></TableCell>
                        <TableCell className="font-mono font-bold text-emerald-600 dark:text-emerald-400">+{parseFloat(p.amount).toLocaleString()} <span className="text-[10px]">{p.currency}</span></TableCell>
            ...
            <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Record Candidate Payment">
                <form onSubmit={handleCandPayment} className="space-y-4">
                    <div>
                        <label className="label dark:text-gray-300 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 block">Candidate</label>
                        <select className="input-field dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" value={candPayForm.candidate_id} onChange={e => setCandPayForm({ ...candPayForm, candidate_id: e.target.value })} required>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="label dark:text-gray-300 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 block">Payment Type</label>
                            <select className="input-field dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" value={candPayForm.payment_type_id} onChange={e => setCandPayForm({ ...candPayForm, payment_type_id: e.target.value })} required>
                        ...
                        <div>
                            ...
                            <input type="number" className="input-field dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 font-mono font-bold" value={candPayForm.amount} onChange={e => setCandPayForm({ ...candPayForm, amount: e.target.value })} required />
                        </div>
                    </div>
                    <div>
                        ...
                        <select className="input-field dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100" value={candPayForm.account_id} onChange={e => setCandPayForm({ ...candPayForm, account_id: e.target.value })} required>
            ...
            <Modal isOpen={isQRModalOpen} onClose={() => setIsQRModalOpen(false)} title="Payment Verification QR">
                <div className="flex flex-col items-center p-6">
                    ...
                    <p className="mt-4 text-gray-500 dark:text-gray-400 text-sm text-center">Scan to verify transaction authenticity</p>
