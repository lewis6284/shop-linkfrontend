                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-white rounded-xl border border-green-100 shadow-sm p-4 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-bold text-green-600">{totalIncome.toLocaleString()} <span className="text-[11px] font-normal">Fbu</span></p>
                            <p className="text-xs text-gray-400">{filteredJournals.filter(j => j.type === 'ENTRY').length} entries</p>
                        </div>
                    </div>
                    {/* Total Expenses */}
                    <div className="bg-white rounded-xl border border-rose-100 shadow-sm p-4 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-bold text-rose-600">{totalExpense.toLocaleString()} <span className="text-[11px] font-normal">Fbu</span></p>
                            <p className="text-xs text-gray-400">{filteredJournals.filter(j => j.type === 'EXIT').length} entries</p>
                        </div>
                    </div>
                    {/* Net Movement */}
                    <div className={`bg-white rounded-xl shadow-sm p-4 flex items-center gap-4 ${netBalance >= 0 ? 'border border-indigo-100' : 'border border-orange-100'}`}>
                        <div className={`p-3 rounded-lg ${netBalance >= 0 ? 'bg-indigo-50' : 'bg-orange-50'}`}>
                            <Activity size={22} className={netBalance >= 0 ? 'text-indigo-600' : 'text-orange-600'} />
                        </div>
                        <div>
                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Net Movement</p>
                            <p className={`text-lg font-bold ${netBalance >= 0 ? 'text-indigo-600' : 'text-orange-600'}`}>
                                {netBalance >= 0 ? '+' : ''}{netBalance.toLocaleString()} <span className="text-[11px] font-normal">Fbu</span>
                            </p>
                            <p className="text-xs text-gray-400">{filteredJournals.length} total entries</p>
                        </div>
                    </div>
                </div>
            )}
